...
export function Bandas() {
  ...
  useEffect(() => {
    const fetchCount = async () => {
      let query = supabase.from("videos_banda").select("*", { count: "exact", head: true });
  ...
  return (
    <div className="p-4 w-full max-w-none">
      ...
      <input
        ...
        className="w-full mb-4 p-3 border-2 border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent"
        value={search}
      ...
      {activeLimit === null && !loading && (
        <p className="text-center py-12 text-gray-500">
          Selecciona cuántos videos deseas ver usando los botones de arriba.
        </p>
      )}
      ...
      {hasMore && !loading && (
        <div className="flex justify-center mt-6">
          <button
            ...
            className="flex items-center gap-2 px-6 py-3 bg-white border-2 border-purple-500 text-purple-600 rounded-xl font-medium hover:bg-purple-50 hover:shadow-md transition-all"
          >
